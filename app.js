/* EMS Academy v46 loader.
   Keeps the existing stable application code from the v45 commit and applies
   only the requested DI/HAD test changes on top. */
(async()=>{
  ...
  try{
    ...
    /* The former HAD exam remains HAD, but is now named correctly. */
    ...
    /* DI now hosts the new 26-question EMS Charter exam. */
    tests.di.title='Экзамен по Уставу EMS';
    tests.di.short='Устав Emergency Medical Service';
    tests.di.description='Проверка знания Устава EMS: обязанности, рабочие ситуации, дисциплина, порядок работы и основные запреты.';
    tests.di.questions=[
    ...
    /* Update visible cards without touching the HTML layout. */
    const diCard=document.querySelector('.test-choice[data-test="di"]');
    const hadCard=document.querySelector('.test-choice[data-test="charter"]');
    if(diCard){
      diCard.querySelector('h3').textContent='Экзамен по Уставу EMS';
      diCard.querySelector('p').textContent='Проверка знания Устава EMS: обязанности, рабочие ситуации, дисциплина, порядок работы и основные запреты.';
      diCard.querySelector('.choice-meta').innerHTML='<span>26 вопросов</span><span>15:00</span><span>80%</span>';
    }
    if(hadCard){
      hadCard.classList.add('had-choice');
      hadCard.querySelector('h3').textContent='Переаттестация';
      hadCard.querySelector('p').textContent='Проверка сотрудников HAD по внутренним требованиям EMS и действующему порядку работы.';
      hadCard.querySelector('.choice-meta').innerHTML='<span>12 вопросов</span><span>15:00</span><span>80%</span>';
    }

    const introBtn=document.getElementById('startIntroBtn');
    if(introBtn) introBtn.textContent='Сдать экзамен по Уставу EMS →';

    /* HAD card — red, like the rest of the HAD styling. */
    const style=document.createElement('style');
    style.textContent=`
      .test-choice.had-choice{border-color:rgba(239,68,68,.22);background:linear-gradient(145deg,rgba(239,68,68,.10),rgba(255,255,255,.025));}
      .test-choice.had-choice .choice-code{color:#ef4444;}
      .test-choice.had-choice .choice-status{color:#fca5a5;}
      .test-choice.had-choice .choice-arrow{color:#ff7b7b;}
      .test-choice.had-choice:hover{border-color:rgba(239,68,68,.45);background:rgba(239,68,68,.12);}
      .test-choice.had-choice.selected{border-color:#ef4444;background:rgba(239,68,68,.14);box-shadow:0 0 28px rgba(239,68,68,.12);}
      .test-choice.had-choice.selected .choice-arrow{color:#fecaca;}
    `;
    document.head.appendChild(style);

    /* The Ustav guide button now opens the DI Ustav exam. */
    document.getElementById('startCharterBtn')?.addEventListener('click',()=>{chooseTest('di');document.getElementById('nameModal').classList.add('show')});
    document.getElementById('startCharterBtn2')?.addEventListener('click',e=>{e.stopPropagation();chooseTest('di');document.getElementById('nameModal').classList.add('show')});

    /* Re-render after replacing the DI/HAD definitions. */
    if(typeof renderTestChooser==='function') renderTestChooser();
  }catch(err){
    console.error('EMS Academy loader error:',err);
  }
})();
